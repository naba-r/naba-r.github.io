...
| التوزيعة          | الأمر                 | هل يعمل Refresh تلقائي؟     | هل يُنصح Refresh يدوي؟ |
| ----------------- | --------------------- | --------------------------- | ---------------------- |
| Arch / Pacman     | `pacman -Syu firefox` | نعم                         | نعم دائمًا             |
| Ubuntu / APT      | `apt install firefox` | لا                          | نعم مهم جدًا           |
| Fedora / DNF      | `dnf install firefox` | غالبًا نعم (metadata check) | أحيانًا                |
| openSUSE / Zypper | `zypper in firefox`   | نعم                         | غالبًا لا تحتاج        |
...
```
pacman:    pacman -Qo /usr/bin/python3
...
`Zsh` أقوى وأكثر قابلية للتخصيص، لكنها تحتاج إلى إعداد أولي وفهمًا أعمق للنظام.
